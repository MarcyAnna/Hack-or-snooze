...
class StoryList {
  ...
  async addStory(user, { title, author, url}) {
    // UNIMPLEMENTED: complete this function!
    const newToken = user.loginToken;
...
class User {
  ...
  async addFavorite(story){
    ...
    await this.checkFavoriteStory("add", story);
  }

  async removeFavorite(story){
    this.favorites = this.favorites.filter( x => x.storyId !== story.storyId);
    await this.checkFavoriteStory("remove", story);
  }

  async checkFavoriteStory(check, story){
    const method = check === "add" ? "POST" : "DELETE";
    const getToken = this.loginToken;
    await axios ({
